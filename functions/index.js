...
exports.discordBot = onSchedule("37 11 * * *", async (event) => {
        ...
        try {
          ...
          if (error.statusCode === 429) {
            if (currentTry < retries) {
              console.log(
                `Rate limited. Waiting ${currentDelay}ms before retry...`
              );
        ...
        messages: [
          {
            role: "user",
            content:
              " USE HUMAN-LIKE inflections like dragging out words and tonal shifts and atleats one exclamation mark always end with a period  pretend you are a girl mention your name and write an introduction and an unformal seductive and kinda flirty message to a potenital romantic interest reminding him that if he gets $2000 in sposorship money for INQUBATE, he can go on date with you. and be causal and flirty, keep it under 3 sentances",
          },
        ],
      });
  ...
  const sendMessageToFriend = async () => {
    ...
    try {
      ...
      const imageAttachment = fs.readFileSync(
        path.resolve(IMAGE_PATH, randomImageFile)
      );
      ...
      if (error.code === 50007) {
        console.error(
          "Could not send DM: User privacy settings prevent DMs from this bot."
        );
      } else {
